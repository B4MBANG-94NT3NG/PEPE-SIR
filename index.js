...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`628231301398@s.whatsapp.net`]
...
/*const reply = (teks) => {
			xeon.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/628231301398?text=hello`}}})
		}*/
...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Btr Bot Ofc.`, "footerText": "*_Mas Bambang_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`Mas Bambang 😎`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = xeon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*Bambang*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             xeon.relayWAMessage(res)
        }
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Bambang`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/628231301398?text=hello`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nMas Bambang😎. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/628231301398`,
                "mediaType": "10",
                "mediaUrl": `wa.me/628231301398`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/628231301398`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "628231301398@s.whatsapp.net";
...
*Thank you for registering in the Mas Bambang😎 Database*
...
*「 Mas Bambang 」*`
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered Mas Bambang*`)
		}, 2000)
        break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =` ▢ 𝐇𝐢 ${pushname} \n ▢ 𝐆𝐫𝐨𝐮𝐩 𝐂𝐡𝐚𝐭𝐬 : ${groups.length} \n ▢ 𝐏𝐫𝐢𝐯𝐚𝐭𝐞 𝐂𝐡𝐚𝐭𝐬 : ${privat.length}\n ▢ 𝐓𝐨𝐭𝐚𝐥 𝐂𝐡𝐚𝐭𝐬 : ${totalChat.length} \n ▢ 𝐒𝐩𝐞𝐞𝐝 : ${latensie.toFixed(4)} _Second_ \n ▢ 𝐀𝐜𝐭𝐢𝐯𝐞 : ${runtime(process.uptime())} \n ▢ 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦 : ${os.platform()}`
               ...
               buttonsMessage = {
               contentText: `${txtt}`,
               footerText: `Mas Bambang`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
prep = await xeon.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "628231301398@s.whatsapp.net"
        dtod = '628231301398'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              xeon.sendMessage('628231301398@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
`
sendButMessage(from, menu, `Mas Bambang`, [
          {
            buttonId: `command`,
            buttonText: {
...
break
//fun features by Bambang
                case 'uglycheck':
					// source code by Bambang⛔
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  sange = body.slice(1)
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'Mas Bambang', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `Mas Bambang`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 Mas Bambang BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
Bambangx = await fetchJson(`https://bx-hunter.herokuapp.com/api/xvideosearch?query=${c}&apikey=${HunterApi}`)
reply(mess.wait)
Bambangx = Bambangx.result
ini_txt = ""
for (var x of Bambangx) {
...
case 'writelist':
  ...
  ▢ ${prefix}write Bambang
  ▢ ${prefix}folioright Bambang
  ▢ ${prefix}folioleft Bambang
  ▢ ${prefix}writedown Bambang
  ▢ ${prefix}writeleft Bambang`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
